...
yum -y remove rsh-server prelink rsh rsync ypserv tftp tftp-server talk-client talk-server telnet-server xinetd >> $AUDITDIR/service_remove_$TIME.log
...
echo "Disabling SNMP..."
yum -y remove net-snmp net-snmp-utils >> $AUDITDIR/service_remove_$TIME.log

echo "Disable X Window System..."
yum remove xorg-x11*

echo "Disable all Unwanted Server..."
yum -y remove avahi-daemon cups dhcp nfs-utils nfs-utils ypbind
...
servicelist=(dhcp avahi-daemon cups nfslock rpcgssd rpcbind rpcidmapd rpcsvcgssd)
...
echo "Permissions on all logfiles..."
...
|                    ======================================                                 |
|This system is the property of C-DAC. It is for authorized use only.                       |
|Users (authorized and unauthorized) have no explicit or implicit expectation of privacy.   |
|Any or all uses of this system and all files on the this system will be intercepted,       |
|monitored, recorded, copied, audited, inspected, and disclosed to C-DAC management, and    |
|law enforcement personnel as well as other authorized agencies.                            |
|By using this system, the user consents to such interception,monitoring, recording to such |
|interception,monitoring, recording, copying, auditing, inspection, and disclosure          |
|at the discretion of C-DAC. Unauthorized or improper use of this system may result in      |
|administrative disciplinary action and civil and criminal penalties.                       |
|By continuing to use this system you indicate the awareness of and consent to these        |
|terms and conditions of use.                                                               |
|LOG OFF IMMEDIATELY if you do not agree to the terms and conditions stated in this warning.|
|                                                                                           |
 \-----------------------------------------------------------------------------------------/
EOF
...
cat > /etc/motd << 'EOF'
CDAC-MUMBAI AUTHORIZED USE ONLY
EOF
...
echo "No legacy '+' entries exist..."
sed -ri '/^\+:.*$/ d' /etc/passwd
sed -ri '/^\+:.*$/ d' /etc/shadow
sed -ri '/^\+:.*$/ d' /etc/group

echo "user umask value is changing...."
umask 0027
